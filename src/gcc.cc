...
std::string get_compiler_command() {
    // Get the original gcc path from the environment variable
    char* gcc_path = std::getenv("_GCC_PATH");
    if (!gcc_path) {
        std::cerr << "Error: _GCC_PATH environment variable not set" << std::endl;
        exit(1);
    }
    return gcc_path;
}
...
int main(int argc, char* argv[]) {
    // Get the original gcc command
    std::string gcc_cmd = get_compiler_command();
    
    // Get additional arguments from iclargs.json
    std::vector<std::string> iclargs = get_iclargs();
    
    // Construct the full command
    std::string cmd = gcc_cmd;
